...
def location(city, zipcode, lat, longg):
    ...
    check_in = db.execute("SELECT zipcode FROM comments JOIN locations ON locations.id = comments.location_id WHERE locations.zipcode = :zipcode", {"zipcode":zipcode}).fetchall()
    print(check_in)
